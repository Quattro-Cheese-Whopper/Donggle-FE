...
export const useAuth = () => {
  ...
  const fetchCurrentUser = async () => {
    ...
    if (isFetchingUser.current) {
      console.log("🚫 이미 내 사용자 정보 조회 중 - 중복 호출 방지");
      return;
    }

    // 로그인되어 있지 않으면 조회하지 않음
    if (!isLoggedIn) {
      console.log("🚫 로그인되지 않음 - 내 사용자 정보 조회 안함");
      return;
    }
    ...
    try {
      console.log("👤 내 사용자 정보 조회 시작...");
      const response = await authService.getCurrentUser();
      const userData = response.data || response;

      console.log("👤 내 사용자 정보:", userData);
      setUser(userData);
    } catch (error) {
      console.error("❌ 내 사용자 정보 조회 실패:", error);
      setUser(null);
    } finally {
