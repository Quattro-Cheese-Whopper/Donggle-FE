...
const CentralClub = () => {
  // 모든 분야 목록 추출 (중복 제거)
  const allCategories = ['전체', ...new Set(sampleClubs.map(club => club.department))];
  
  // 선택된 분야 상태
  const [selectedCategory, setSelectedCategory] = useState('전체');
  
  // 필터링된 동아리 목록
  const [filteredClubs, setFilteredClubs] = useState(sampleClubs);
  ...
  useEffect(() => {
    if (selectedCategory === '전체') {
      ...
      setFilteredClubs(sampleClubs.filter(club => club.department === selectedCategory));
    }
  }, [selectedCategory]);
